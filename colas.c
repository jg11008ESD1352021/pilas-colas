...
/*RESERVA DE MEMORIA PARA LA COLA Y EL DATO DE CADA ELEMENTO DE LA COLA*/
...
int quitar_cola (Cola * serie){
  ...
  if (serie->tamanio == 0){return -1;}

  sup_elemento = serie->inicio;
  serie->inicio = serie->inicio->siguiente;

  free (sup_elemento->dato);//liberacion de memoria usada por el dato
  free (sup_elemento);//liberacion de memoria usada por la cola

  serie->tamanio--;//reduccion de tamaño
  return 0;
}
...
int main(){
        Cola *serie;//declaracion de la cola
        inicializacion_cola(serie);//inicializacion de la cola
/*INGRESE DE ELEMENTOS A LA COLA*/
        ...
        imprimir_cola(serie);
        printf("\n");
/*EXTRACCION	DE ELEMENTOS A LA COLA*/
        quitar_cola(serie);
        quitar_cola(serie);
        quitar_cola(serie);
        quitar_cola(serie);
